...
- [~] **[high]** settlement / city / road rec detail strings + rationale line  (partial: per-roll detail done via _perRoll; port/fog/plan suffixes + rationale line deferred) + settle#3 + road port suffix (tests/js/recommender.detail_suffix.test.mjs); rationale line + supports-plan still open
